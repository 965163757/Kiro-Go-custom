...
// GetOutboundProxy returns the configured proxy URL for all outbound requests.
func GetOutboundProxy() string {
	cfgLock.RLock()
	defer cfgLock.RUnlock()
	if cfg == nil {
		return ""
...
// UpdateOutboundProxy updates the proxy URL used by outbound HTTP clients.
func UpdateOutboundProxy(proxyURL string) error {
	cfgLock.Lock()
	defer cfgLock.Unlock()
	cfg.OutboundProxy = proxyURL
	return Save()
}
